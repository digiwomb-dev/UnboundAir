...
It may be enabled only after unsuccessful attempts with qwen3-coder-30b, devstral-small, and gpt-oss-20b. A primary agent must first report the three attempts, their model names, approaches, and concrete blockers, then obtain explicit user approval. The selected cloud model must differ from the primary session model.
