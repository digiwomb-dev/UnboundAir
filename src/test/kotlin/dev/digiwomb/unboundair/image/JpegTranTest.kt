...
class JpegTranTest {
    ...
    @Test
    fun `SV-01 crop keeps a 16x8 window with its 3 components intact`() {
        ...
        assertThat(info.width).`as`("width of the crop window").isEqualTo(16)
        assertThat(info.height).`as`("height of the crop window").isEqualTo(8)
        assertThat(info.components).`as`("the crop keeps all 3 stored components").isEqualTo(3)
    }

    @Test
    fun `SV-03 grayscale reduces the stored components to a single luma component`() {
        val source = TestImages.copy("envelope_dl_300dpi_raw.jpg", dir)
        ...
        val info = JpegInfo.read(target)
        assertThat(info.components).`as`("grayscale leaves exactly one component").isEqualTo(1)
    }

    @Test
    fun `SV-01 cropping a file that is not a JPEG fails with a JpegTranException naming the program`() {
        val nonJpeg = dir.resolve("not_a_jpeg.bin")
        Files.write(nonJpeg, byteArrayOf(0, 1, 2, 3, 4, 5, 6, 7))
        val target = dir.resolve("cropped.jpg")

        assertThatThrownBy { JpegTran.crop(nonJpeg, target, 16, 8, 0, 0) }
            .isInstanceOf(JpegTranException::class.java)
            .hasMessageContaining("jpegtran")
    }
}
